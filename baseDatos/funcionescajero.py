...
#metodo para realizar un retiro de la tarjeta del cliente
def retirar_saldoTarjetaCliente(monto,IDusuario, num_tarjeta):
    saldo= Ver_Saldo_Tajerta(IDusuario,num_tarjeta)
    FondosCuenta = saldo[0] - int(monto)
    consulta = "UPDATE UsuarioxTarjeta set monto  = ? WHERE IDusuario = ? and numero_tarjeta = ?"
    parametros= (FondosCuenta,IDusuario,num_tarjeta)
    conexion = establecer_conexion()
    cursor=conexion.cursor() 
...
#metodo para realizar retiros al saldo del cliente
def retirar_saldoCliente(monto,IDusuario):
    saldo= Ver_Saldo_Usuario(IDusuario)
    FondosCuenta =  saldo[0]-int(monto)
    consulta = "UPDATE Usuarios set saldo  = ? WHERE IDusuario = ?"
    parametros= (FondosCuenta,IDusuario,)
...
def Ver_Saldo_cajero(IDcajero):
    consulta = "SELECT monto FROM cajeros WHERE IDcajero = ?"
    Parametros = (IDcajero,)
    conexion = establecer_conexion()
    if conexion is not None:
        cursor=conexion.cursor()    
...
def modificar_saldoCajero(monto,IDcajero):
    saldo_Cajero = Ver_Saldo_cajero(IDcajero)
    Monto_cajero = int(saldo_Cajero[0]) - int(monto)
    consulta = "UPDATE Cajeros set monto  = ? WHERE IDcajero = ?"
    parametros= (Monto_cajero,IDcajero,)
    conexion = establecer_conexion()
    cursor=conexion.cursor() 
    cursor.execute(consulta,parametros)
    conexion.commit()
